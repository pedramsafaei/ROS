...
Copyright(c) 2014 Pedram Safaei
...
# Subscriber for encoders
        self._Left_Encoder_ = rospy.Subscriber(
            'left_encoder', Float32, self._Update_Left_Encoder)

        self._Right_Encoder_ = rospy.Subscriber(
            'right_encoder', Float32, self._Update_Right_Encoder)
...
#	        self._cmd_velocity_ = rospy.Subscriber("cmd_vel", Twist, self._HandleVelocityCommand) # Is this line or the below bad redundancy?
        ...
        self._cmd_velocity_ = rospy.Subscriber(
            "cmd_vel_mux/input/teleop", Twist, self._HandleVelocityCommand)

# Publish speed

        self._left_motor = rospy.Publisher(
            "left_wheel_speed", Int32, queue_size=1)
        self._right_motor = rospy.Publisher(
            "right_wheel_speed", Int32, queue_size=1)
...
# Replacement of Robot params
        ...
        self.distance_per_count = (
            math.pi * self.wheel_diameter) / self.counts_per_revolution  # //0.00003366
        self.radians_per_count = (
            self.distance_per_count) / self.trackwidth  # //0.003365992
...
# Subscribe for time
        ...
        self._micro_sec_ = rospy.Subscriber(
            'micro_sec', UInt64, self._Update_Micro_Sec)
...
# 3
    ...
    def _Compute_Wheel_Odometry(self):
        ...
        try:
            ...
            deltaDistance = 0.5 * \
                (delta_Left + delta_Right) * self.distance_per_count
...
#		rospy.logwarn("Sending speed command message: " + message)
        ...
        if (v > 0):
            if ((speed_limit * self.distance_per_count) - math.fabs(angularVelocityOffset) < v):
                v = (speed_limit * self.distance_per_count) - \
                    math.fabs(angularVelocityOffset)
        elif(v < 0):
            if (-((speed_limit * self.distance_per_count) - math.fabs(angularVelocityOffset)) > v):
                v = -((speed_limit * self.distance_per_count) -
                      math.fabs(angularVelocityOffset))
...
#		rospy.loginfo(NormalizedRightCV)
        ...
        actual_left_speed = ((old_left_value - old_min) /
                             (old_max - old_min)) * (new_max - new_min) + new_min

        actual_right_speed = ((old_left_value - old_min) /
                              (old_max - old_min)) * (new_max - new_min) + new_min
